...
function List({ items, addFolderClick, isRemovable, onRemoveFolder, onClickFolder }) {
    ...
    return (
        <ul onClick={addFolderClick} className={Styles.list}>
            {items.map((item, index) => (
                <li key={index} onClick={onClickFolder ? () => onClickFolder(item) : null} className={(item.className ? Styles[item.className] : (Styles.active))}>
                    <i>
                        {item.icon ? (item.icon) : (
